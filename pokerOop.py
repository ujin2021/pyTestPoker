...
suits = 'CDHS'
ranks = '23456789TJQKA'
hand_ranking = {'HIGH_CARD' : 0, 'ONE_PAIR' : 1, 'TWO_PAIRS' : 2, "THREE_OF_A_KIND" : 3, "STRAIGHT" : 4, "FLUSH" : 5, "FULL_HOUSE" : 6, "FOUR_OF_A_KIND" : 7, "STRAIGHT_FLUSH" : 8}
'''
...
class Hands:
    ...
    def is_flush(self):
        ...
        suit_set = set()
        for i in range(5):
            suit_set.add(self.cards[i][1]) #card's suit set
        if (len(suit_set) == 1):
            return [x[0] for x in self.cards] #card rank list
        else:
            return None
    ...
    def is_straight(self):
        ...
        for i in range(0, 4):
            check = [PKCard.value(card) for card in self.cards]
            if (check == [14, 5, 4, 3, 2]): #baby straight
                return ['STRAIGHT', self.cards[1][0]]
            elif (check[i] - 1 == check[i+1]): # check if rank if decreasing
                continue
            else:
                return None

        return ['STRAIGHT', self.cards[0][0]] #제일 큰 수를 가진 카드의 rank

    def classify_by_rank(self):
        result = dict() #{rank:[suit1, suit2], ...}

        for i in range(5):
            if (self.cards[i][0] in result):
                temp_list = [] #initialize temp list
                if (type(result[self.cards[i][0]]) == list): #rank당 suit여러개
                    temp_list.extend(result[self.cards[i][0]])
                else:
                    temp_list.append(result[self.cards[i][0]]) #rank당 suit 한개
                temp_list.append(self.cards[i][1])
                result[self.cards[i][0]] = temp_list
            else:
                result[self.cards[i][0]] = self.cards[i][1]

        return result

    def find_a_kind(self):
        self.cards_by_ranks = self.classify_by_rank()
        ...
        if (len(rank_keys) == 2):  # full house or four of a kind
            if (type(rank_values[0]) == list and type(rank_values[1]) == list): #full house
                ...
                result = ["FULL_HOUSE", temp]
            else: #four of a kind
                if(type(rank_values[0]) == list):
                    temp = [rank_keys[0], rank_keys[1]]
                else:
                    temp = [rank_keys[1], rank_keys[0]]
                result = ["FOUR_OF_A_KIND", temp]
            ...
            for i in range(4):
                if(type(rank_values[i]) == list):
                    ...
                    temp.append(rank_keys[i])
            result = ["ONE_PAIR", temp]
        else: #len(rank_keys) == 5 -> straight or high card
            if(rank_keys == ['A', '5', '4', '3', '2']):
                result = 0 #Straight
            else:
                result = ["HIGH_CARD", rank_keys] #high card, return rank list

        return result

    def tell_hand_ranking(self):
        isFlush = self.is_flush() #'None' or rank list
        isStraight = self.is_straight() #None' or highest rank
        findAkind = self.find_a_kind() #None' or result list
        ...
        if (type(isFlush) != list and type(isStraight) != list and type(findAkind) == list):
            ...
            return ["STRAIGHT_FLUSH", isStraight[1]]
        elif (type(isFlush) != list and type(isStraight) == list):
...
# print(Hands([PKCard('AD'), PKCard('5D'), PKCard('4C'), PKCard('3D'), PKCard('2D')]).is_straight())
# print(Hands([PKCard('KD'), PKCard('JD'), PKCard('QC'), PKCard('AD'), PKCard('TD')]).tell_hand_ranking())
# print(Hands([PKCard('4D'), PKCard('4C'), PKCard('4C'), PKCard('AS'), PKCard('AD')]).find_a_kind())
'''
